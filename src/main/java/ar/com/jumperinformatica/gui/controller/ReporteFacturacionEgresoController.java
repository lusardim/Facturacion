...
public class ReporteFacturacionEgresoController extends ReporteFacturacionController {
	
	public ReporteFacturacionEgresoController(Frame padre,
			FacturacionBean facturacionBean) {
		super(padre, facturacionBean);
	}

	
	public ReporteFacturacionEgresoController(JDialog padre,
			FacturacionBean facturacionBean) {
		super(padre, facturacionBean);
	}
	...
	protected void initModels() {
		...
		locParametroTipoFactura[1] = new ParametroTipoFactura("Factura B",TipoFactura.B);
		locParametroTipoFactura[2] = new ParametroTipoFactura("Factura A y B",TipoFactura.A, TipoFactura.B);
	...
	public String toString() {
		return "Reporte Facturación Egreso";
	}
	...
	public void aceptar() {
		try {
			...
			GestorImpresion.imprimirConViewer(this.getVista(),Reportes.FACTURACION_EGRESO, locListaParametros);
		}
	...
	private Map<String, Object> getListaParametros() {
		try{
			...
			List<ReporteFacturacion> locListaReporte = this.getFacturacionBean().findListaFacturasEgreso(
			...
			for (ReporteFacturacion cadaReporte : locListaReporte){
				cadaReporte.setDtype("FacturaEgreso");
